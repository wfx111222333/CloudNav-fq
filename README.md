...
#### 最新版本v1.8.2 (2026.7.1)
- 1、图标上传与管理：链接编辑器新增图标上传功能，客户端自动压缩至64px（PNG保留透明度），存储到文件管理系统的"图标文件夹"
- 2、文件传输助手：消息默认仅显示最近7天，向上滚动自动加载历史消息，滚动位置保持不变
- 3、文件管理系统分页：网格视图和列表视图均支持分页（每页3行），支持上/下页切换
- 4、文件管理缩略图优化：图片显示使用压缩缩略图，点击预览和下载时加载原图
- 5、R2目录镜像：上传文件时R2存储键包含文件夹前缀（如"图标文件夹/xxx.png"），目录结构与文件管理系统一致
- 6、文件列表独立加载：文件管理标签页独立获取全量文件列表，不受消息7天显示限制
- 7、修复部署问题：文件服务路由从catch-all改为双段路由（`[name].ts` + `[folder]/[name].ts`），修复Cloudflare Pages Functions构建失败
